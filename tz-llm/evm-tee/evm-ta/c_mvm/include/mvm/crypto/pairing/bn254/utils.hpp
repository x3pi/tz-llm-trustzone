...
inline std::string to_hex_string(const intx::uint256& val) {
    if (val == 0) {
        return "0x0"; // Xử lý riêng trường hợp số 0
    }
    // Sử dụng hàm hex() được cung cấp bởi thư viện intx
    // Hàm intx::hex() thường không bao gồm tiền tố "0x"
    return "0x" + intx::hex(val);
}
// Hàm chuyển đổi Fq sang chuỗi hex
// QUAN TRỌNG: Hàm này giả định lớp Fq có phương thức `value_non_montgomery()`
// để lấy giá trị số học thực tế (không phải dạng Montgomery).
// Nếu lớp Fq của bạn dùng cách khác, bạn cần điều chỉnh cho phù hợp.
inline std::string to_hex_string(const Fq& val) {
    const intx::uint256 montgomery_val = val.value();
    intx::uint256 non_mont_val;
    try {
        // Giả định ModArith có phương thức from_mont()
        non_mont_val = BaseFieldConfig::MOD_ARITH.from_mont(montgomery_val);
    } catch (...) {
         std::cerr << "Warning: Could not convert Fq from Montgomery. Printing raw/Montgomery value." << std::endl;
         non_mont_val = montgomery_val;
    }
    // Gọi hàm to_hex_string(uint256) đã được sửa đúng ở trên
    return to_hex_string(non_mont_val);
}


// Hàm chuyển đổi Fq2 sang chuỗi hex
inline std::string to_hex_string(const Fq2& val) {
    std::stringstream ss;
    // Gọi to_hex_string cho từng thành phần Fq
    ss << "(" << to_hex_string(val.coeffs[0]) << ", " << to_hex_string(val.coeffs[1]) << "u)";
    return ss.str();
}

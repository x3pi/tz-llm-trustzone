...
public:
    ...
    friend constexpr uint operator<<(uint x, T shift) noexcept
    {
...
constexpr unsigned clz(T x) noexcept
{
    ...
    if constexpr (sizeof(T) <= sizeof(unsigned int))
        return __builtin_clz(static_cast<unsigned int>(x)) - (sizeof(unsigned int) - sizeof(T)) * 8;
    else if constexpr (sizeof(T) <= sizeof(unsigned long))
        return __builtin_clzl(static_cast<unsigned long>(x)) - (sizeof(unsigned long) - sizeof(T)) * 8;
    else
...
public:
    ...
    template <typename T, typename = std::enable_if_t<std::is_integral_v<T>>>
    friend constexpr uint operator<<(const uint& x, T shift) noexcept
    {
        static_assert(sizeof(shift) <= sizeof(uint64_t));
        return x << static_cast<uint64_t>(shift);
    }
...
constexpr uint64_t udivrem_by1(uint64_t u[], int len, uint64_t d) noexcept
{
    ...
    while (true)
    {
        const auto res = udivrem_2by1({*it, rem}, d, reciprocal);
        *it = res.quot;
        rem = res.rem;
        if (it == &u[0])
            break;
...
constexpr uint128 udivrem_by2(uint64_t u[], int len, uint128 d) noexcept
{
    ...
    while (true)
    {
        const auto res = udivrem_3by2(rem[1], rem[0], *it, d, reciprocal);
        *it = res.quot;
        rem = res.rem;
        if (it == &u[0])
            break;
...
constexpr bool add(uint64_t s[], const uint64_t x[], const uint64_t y[], int len) noexcept
{
    ...
    for (int i = 0; i < len; ++i)
    {
        const auto res = addc(x[i], y[i], carry);
        s[i] = res.value;
        carry = res.carry;
    }
    return carry;
    ...
    uint64_t q[], uint64_t u[], int ulen, const uint64_t d[], int dlen) noexcept
{
    ...
    for (int j = ulen - dlen - 1; j >= 0; --j)
    {
        ...
        {
            const auto res3 = udivrem_3by2(u2, u1, u0, divisor, reciprocal);
            qhat = res3.quot;
            const auto rhat = res3.rem;

            const auto overflow = submul(&u[j], &u[j], d, dlen - 2, qhat);
            const auto sc1 = subc(rhat[0], overflow);
            u[j + dlen - 2] = sc1.value;
            const auto sc2 = subc(rhat[1], sc1.carry);
            u[j + dlen - 1] = sc2.value;
            const bool carry = sc2.carry;

...
std::vector<uint8_t> ecrecover(const std::vector<uint8_t> &message_hash,
  ...
  // Mocked for TEE RAM EVM to avoid libsecp256k1 dependency.
  return std::vector<uint8_t>(20, 0); // Dummy 20-byte address
}
...
mvm::Code MyExtension::Math(mvm::Code input) {
  // Mocked for TEE RAM EVM to avoid GMP/MPFR dependencies.

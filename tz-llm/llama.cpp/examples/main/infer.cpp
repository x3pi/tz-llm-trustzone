...
int infer(int argc, char **argv, char *input, size_t input_len, char *output,
          size_t output_len) {
  // lcf: check whether the first token is calculated
  bool _first_token_calculated = false;
  int64_t _ttft_start = -1;
  int64_t _ttft_end = -1;
  ...
  llama_backend_init();
  const int64_t backend_init = ggml_time_us();
  ...
  // zzh: preparation stage end
  // load the model and apply lora adapter, if any
  LOG_INF("%s: load the model and apply lora adapter, if any\n", __func__);
  llama_init_result llama_init = llama_init_from_gpt_params(params);
  // zzh: llama init models end
  const int64_t llama_model_init = ggml_time_us();
  std::cout << "zzh: Llama model init took " << llama_model_init - backend_init
            << " us" << std::endl;
  ...
  const uint64_t param_parse = ggml_time_us();
  std::cout << "zzh: Param parse took " << param_parse - llama_model_init
            << " us" << std::endl;
  ...
  // llama thread pool end
  const uint64_t process_thread_init = ggml_time_us();
  std::cout << "zzh: Process & thread init took "
            << process_thread_init - param_parse << " us" << std::endl;
  ...
  const int64_t create_session_end = ggml_time_us();
  std::cout << "zzh: Create session took "
            << create_session_end - process_thread_init << " us" << std::endl;
...

  {
    ...
    if (params.interactive_first || !params.prompt.empty() ||
      ...
      std::cout << "zzh: tokenize the prompt" << std::endl;
      embd_inp = ::llama_tokenize(ctx, prompt, true, true);
    } else {
      LOG_DBG("use session tokens\n");
      embd_inp = session_tokens;
      std::cout << "zzh: use session tokens" << std::endl;
    }
  ...
  if (embd_inp.empty()) {
    if (add_bos) {
      ...
      LOG_WRN("embd_inp was considered empty and bos was added: %s\n",
              string_from(ctx, embd_inp).c_str());
      std::cout << "zzh: bos was added in embd_inp" << std::endl;
    } else {
  ...
  // zzh: llama handle reuse session code end
  const int64_t tokenize_end = ggml_time_us();
  std::cout << "zzh: Tokenize took " << tokenize_end - create_session_end
            << " us" << std::endl;
  ...
  // zzh: sampler end
  const int64_t sampler_init_end = ggml_time_us();
  std::cout << "zzh: Sampler init took " << sampler_init_end - tokenize_end
            << " us" << std::endl;
  ...
  if (llama_model_has_encoder(model)) {
    std::cout << "zzh: has encoder" << std::endl;
  ...
  // zzh: encode end
  const int64_t encode_end = ggml_time_us();
  std::cout << "zzh: Encode took " << encode_end - sampler_init_end
            << " us, now into main loop" << std::endl;
  ...
  while ((n_remain >= 0 && !is_antiprompt) || params.interactive) {
    ...
    embd.clear();
    const int64_t predict_end = ggml_time_us();
    // std::cout << "zzh: Predict took " << predict_end - loop_start << " us"
    //          << std::endl;
    ...
    if (input_echo && display) {
      ...
      for (auto id : embd) {
        ...
        // lcf: here each token will be printed to the console, maybe we can try
        //     to calculate TTFT after this line of code
        LOG("%s", token_str.c_str());
        if (!_first_token_calculated && !is_first_input) {
          _first_token_calculated = true;
          _ttft_end = ggml_time_us();
          LOG("\nlcf: time to first token: %lu us\n", _ttft_end - _ttft_start);
        }
    ...
    if ((int)embd_inp.size() <= n_consumed) {
      ...
      if (llama_token_is_eog(model, gpt_sampler_last(smpl))) {
        ...
        if (params.interactive) {
          ...
          std::cout << std::endl;
          std::cout << "output_ss: " << output_ss.str() << std::endl;
          std::strncpy(output, assistant_ss.str().c_str(), output_len);
      ...
      // if current token is not EOG, we add it to current assistant
      // message
      if (params.conversation) {
        const auto id = gpt_sampler_last(smpl);
        assistant_ss << llama_token_to_piece(ctx, id, false);
        // append the token to the char * output
        std::string token = llama_token_to_piece(ctx, id, false);
        int output_len = std::strlen(output);
        std::strncpy(output + output_len, token.c_str(), token.size());
        output_len += token.size();
        // msync(output + output_len, token.size(), MS_SYNC | MS_INVALIDATE);

        // std::cout << "token length this time : " << token.size() << std::endl;
        output[output_len] = '\0';
      }
      ...
      if (n_past > 0 && is_interacting) {
        ...
        // do {
        //     another_line = console::readline(line,
        //     params.multiline_input); buffer += line;
        // } while (another_line);

        // use char * input and input_len to value the buffer
        // input is the func argument, input_len is the length of the
        // input

        std::cout << "User input is: " << input << std::endl;
        std::cout << "User input length is: " << input_len << std::endl;

        // only take the input once
        if (is_first_input) {
          buffer = std::string(input, input_len);
          is_first_input = false;
        } else {
          goto ret;
        }

        // record the beginning of TTFT
        _ttft_start = ggml_time_us();
        ...
        if (buffer.length() > 1) {
          ...
          // implementation is that we can't distinguish between user
          // input and special tokens (prefix/postfix)
          const auto line_pfx =
...
ret:
  ...
  std::cout << "Now quit the function `infer`\n";
  console::cleanup();

                          ...
                          const std::vector<llama_token> &output_tokens)
{
    ...
    yaml_dump_non_result_info(
        logfile, params, ctx, timestamp, input_tokens, model_desc);
...
extern void parse_prompt(gpt_params & params, const std::string & value);

int main(int argc, char **argv)
{

    // extern void matmul_perf_test(int m, int k, int n);
    // matmul_perf_test(128, 2048, 4096);
    // matmul_perf_test(128, 1024, 4096);
    // matmul_perf_test(128, 512, 4096);

    // stdout defaults to fully-buffered when not attached to a real TTY
    // (e.g. run over a UART shell / hdc), which reorders/garbles output
    // relative to unbuffered LOG()/printf writers - see fake_ca.cpp's
    // identical fix for the same symptom.
    ...
    extern void *cmd_queue_addr;
    cmd_queue_addr = vaddr;
    all_ring_buffer *task_queue = (all_ring_buffer *)cmd_queue_addr;
    ...
    printf("\nraw n bytes: ");
    for (int i = 0; i < 16; i++) printf("%02x ", (unsigned char)task_queue->n[i]);
    printf("\n");
    fflush(stdout);
    params.cache = std::atoi(task_queue->cache_p);
    parse_prompt(params, std::string(task_queue->prompt));
    params.model = std::string(task_queue->inner_model_path);
    printf("raw n bytes (post parse_prompt): ");
    for (int i = 0; i < 16; i++) printf("%02x ", (unsigned char)task_queue->n[i]);
    printf("\n");
    fflush(stdout);
    ...
    std::cout << "before set_io_model_path" << std::endl;
    set_io_model_path(params.io_model_path.c_str());
    std::cout << "after set_io_model_path" << std::endl;
    extern void set_cache_proportion(int p);
    std::cout << "before set_cache_proportion" << std::endl;
    ...
    std::cout << "before console init" << std::endl;
    ...
    std::cout << "before llama_init_from_gpt_params" << std::endl;
    // load the model and apply lora adapter, if any
    LOG_INF("%s: load the model and apply lora adapter, if any\n", __func__);
    std::cout << "before llama_init_from_gpt_params" << std::endl;
    llama_init_result llama_init = llama_init_from_gpt_params(params);
    std::cout << "after llama_init_from_gpt_params" << std::endl;
    ...
    std::cout << "before llama_init_sampler" << std::endl;
    ...
    std::cout << "before ggml_threadpool_new" << std::endl;
    ...
    if (n_ctx > n_ctx_train) {
        LOG_WRN(
            "%s: model was trained on only %d context tokens (%d specified)\n",
            __func__,
            n_ctx_train,
            n_ctx);
    }
    ...
    std::cout << "before gpt_sampler_new" << std::endl;
    ...
    {
        auto prompt =
            (params.wrap_user_chat && params.enable_chat_template
             && !params.prompt.empty()) ?
                chat_add_and_format(
                    model, chat_msgs, "user", params.prompt) // wrap a
                                                              // literal
                                                              // -t/--text
                                                              // prompt as
                                                              // a single
            ...
            (params.conversation && params.enable_chat_template
             && !params.prompt.empty()) ?
                chat_add_and_format(
                    model, chat_msgs, "system", params.prompt) // format
                                                               // the
                                                               // system
                                                               // prompt
                                                               // in
    ...
    std::cout << "before llama_token_bos" << std::endl;
    ...
    std::cout << "before llama_kv_cache_seq_rm" << std::endl;
    ...
    for (const std::string &antiprompt : params.antiprompt) {
        antiprompt_ids.emplace_back(
            ::llama_tokenize(ctx, antiprompt, false, true));
    }
    ...
    if (llama_model_has_encoder(model)) {
        ...
        if (llama_encode(
                ctx,
                llama_batch_get_one(enc_input_buf, enc_input_size, 0, 0))) {
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (!embd.empty()) {
            ...
            if ((int)embd.size() > max_embd_size) {
                ...
                LOG_WRN("<<input too long: skipped %d token%s>>",
                        skipped_tokens,
                        skipped_tokens != 1 ? "s" : "");
            ...
            if (ga_n == 1) {
                ...
                while (n_past >= ga_i + ga_w) {
                    ...
                    llama_kv_cache_seq_div(
                        ctx, 0, ga_i + ib * bd, ga_i + ib * bd + ga_w, ga_n);
                    llama_kv_cache_seq_add(
                        ctx, 0, ga_i + ib * bd + ga_w, n_past + ib * bd, dd);
            ...
            if (!embd.empty() && !path_session.empty()) {
                session_tokens.insert(
                    session_tokens.end(), embd.begin(), embd.end());
        ...
        if ((int)embd_inp.size() <= n_consumed && !is_interacting) {
            ...
            while ((int)embd_inp.size() > n_consumed) {
                ...
                // grammar rules
                gpt_sampler_accept(
                    smpl, embd_inp[n_consumed], /* accept_grammar= */ false);
        ...
        if ((int)embd_inp.size() <= n_consumed) {
            ...
            if (!params.antiprompt.empty()) {
                ...
                const std::string last_output =
                    gpt_sampler_prev_str(smpl, ctx, n_prev);
                ...
                for (std::string &antiprompt : params.antiprompt) {
                    ...
                    size_t search_start_pos =
                        last_output.length() > static_cast<size_t>(
                            antiprompt.length() + extra_padding) ?
                            last_output.length()
            ...
            if (llama_token_is_eog(model, gpt_sampler_last(smpl))) {
                ...
                if (params.interactive) {
                    ...
                    if (params.enable_chat_template) {
                        chat_add_and_format(
                            model, chat_msgs, "assistant", assistant_ss.str());
                    }
            ...
            if (n_past > 0 && is_interacting) {
                ...
                if (buffer.length() > 1) {
                    ...
                    bool format_chat = params.conversation
                                       && params.enable_chat_template;
                    std::string user_inp =
                        format_chat ?
                            chat_add_and_format(
                                model, chat_msgs, "user", std::move(buffer)) :
                            std::move(buffer);
                    ...
                    if (need_insert_eot && format_chat) {
                        ...
                        embd_inp.push_back(eot == -1 ? llama_token_eos(model) :
                                                       eot);
                        need_insert_eot = false;
                    }

                    embd_inp.insert(
                        embd_inp.end(), line_pfx.begin(), line_pfx.end());
                    embd_inp.insert(
                        embd_inp.end(), line_inp.begin(), line_inp.end());
                    embd_inp.insert(
                        embd_inp.end(), line_sfx.begin(), line_sfx.end());
    ...
    std::cout << "===GENERATED_ANSWER_START===" << std::endl;

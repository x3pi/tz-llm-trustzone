...
#define GGML_RKNPU2_NAME "RKNPU2"
#define GGML_RKNPU2_MAX_DEVICES 1
// backend API
GGML_API GGML_CALL ggml_backend_t ggml_backend_rknpu2_init(int32_t device);
...
int ggml_rknpu2_can_mul_mat_b(const ggml_tensor * tensor);
int ggml_rknpu2_transform_tensor(const void * data, ggml_tensor * tensor, size_t offset, size_t size);
...
GGML_API GGML_CALL ggml_backend_buffer_type_t ggml_backend_rknpu2_buffer_type(int32_t dev_num);

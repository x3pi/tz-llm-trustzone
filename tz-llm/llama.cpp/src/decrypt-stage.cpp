...
struct decrypt_ctx {
  EVP_CIPHER_CTX *ctx;
  decrypt_ctx(const unsigned char *key, const unsigned char *iv) {
    ctx = EVP_CIPHER_CTX_new();
  }
  ~decrypt_ctx(void) { EVP_CIPHER_CTX_free(ctx); }
};
...
void decrypt_blocks(const unsigned char *ciphertext, unsigned char *plaintext,
                    const unsigned char *key, const unsigned char *iv,
                    int start, int end) {
  auto ctx = dctx.ctx;
  ...
  int len;
  GGML_ASSERT(
      EVP_DecryptUpdate(ctx, plaintext, &len, ciphertext, end - start) == 1);
...
public:
  ...
  DecryptTask(void *buf, size_t count) : buf(buf), count(count) {}
...
extern "C" int usys_config_tzasc(int rgn_id, unsigned long base_addr,
                                 unsigned long top_addr);

unsigned long cur_addr[4] = {0x0000000100000000, 0x0000000190000000,
                             0x0000000220000000, 0x00000002b0000000};
std::priority_queue<std::pair<unsigned long, unsigned long>> pending_addr[4];
// BUG FIX: commit_tzasc() is called from DecryptStage::start(), which runs
// whenever ANY pipeline (one per tensor) finishes IOStage and transitions
// to DecryptStage. Pipeline::finish_stage() only serializes access to that
// ONE pipeline's own state (per-instance stage_mtx) -- it provides no
// synchronization across DIFFERENT pipelines, and with ~200 tensors
// sharing just 4 cma_index values, many different tensors' threads call
// commit_tzasc() for the SAME cma_index concurrently in practice. Both
// pending_addr[] (std::priority_queue, not thread-safe) and cur_addr[]
// were being pushed/popped/read from multiple threads with zero locking -
// a genuine data race that can corrupt the priority_queue's internal heap,
// silently breaking the "only commit strictly contiguous, in-order
// ranges" invariant the base_addr == cur_addr[cma_index] check depends on.
// When that happens, a pending range can get stuck behind a corrupted/
// reordered entry and never reach usys_config_tzasc() at all -- the
// tensor's Pipeline still reports is_finished()==true (that bookkeeping
// is separately counter-based and unaffected), but the TZASC boundary was
// never actually extended to cover its real data, so whatever reads
// tensor->data afterward gets wrong/stale bytes. This is the most likely
// explanation for the "process reaches completion but produces empty/
// corrupted output" failure mode (see STATUS.md, Bug #2 continuation).
std::mutex commit_tzasc_mtx[4];

void commit_tzasc(int cma_index, unsigned long _base_addr,
                  unsigned long _top_addr) {
  std::lock_guard<std::mutex> _(commit_tzasc_mtx[cma_index]);
  ...
  while (!pending_addr[cma_index].empty()) {
    ...
    usys_config_tzasc(8 + cma_index, base_addr, top_addr);
  }
}
#endif
...
void DecryptStage::start(void *input) {
  ...
  for (int cma_index = 0; cma_index < msg->cma_region.size(); cma_index++) {
    if (!is_strawman) {
      commit_tzasc(cma_index, msg->cma_region[cma_index].first,
...
std::pair<std::shared_ptr<Task>, bool> DecryptStage::get_task(void *) {
  ...
  auto task = std::make_shared<DecryptTask>(
      buf + submit_pos, std::min(BLOCK_SIZE, size - submit_pos));
...
void *DecryptStage::get_msg(void) { return buf; }

...
/*
 * Default (include/mmc.h) is 65535 sectors (~32MiB), the hardware/protocol
 * maximum for a single READ_MULTIPLE_BLOCK transfer. Any FIT component
 * larger than that (the Linux kernel Image, ~38.7MiB here) is split into a
 * first chunk of exactly this max size plus a remainder chunk. That specific
 * split -- one DMA transfer running at the absolute maximum allowed size --
 * has been observed to reproducibly corrupt data on this board (deterministic
 * wrong hash on kernel Image load, on multiple different SD cards, while
 * every other FIT component -- all under 32MiB, always read in one
 * comfortably-sized transfer -- has never once failed). Shrinking the max
 * per-transfer chunk avoids ever hitting that maximum-size DMA transfer.
 */
...
#define ENV_MEM_LAYOUT_SETTINGS \
	...
	"fdtoverlay_addr_r=0xe0000000\0" \
	"fdt_addr_r=0xe0100000\0" \
	"kernel_addr_r=0x00400000\0" \
	"kernel_addr_c=0x05480000\0" \
	"ramdisk_addr_r=0xe4000000\0"

			      ...
			      struct spl_image_info *image_info)
{
	...
	if (!fit_image_verify_with_data(fit, node, src, length)) {
		/*
		 * A hash mismatch here can be caused by silent SD/MMC read
		 * corruption rather than an actually-bad image: mmc_bread()'s
		 * own retry loop only re-issues a command when the MMC
		 * controller itself reports an error, not when the data
		 * silently comes back wrong but the command "succeeds". For
		 * large external-data images (e.g. a multi-tens-of-MB OP-TEE
		 * blob) that gap is the only unprotected step in the whole
		 * read path, so retry the read+verify here before failing.
		 * Embedded-data images are already fully resident in the FIT
		 * blob read earlier and have nothing left to re-read.
		 */
		bool verified = false;
		...
		if (external_data) {
			...
			for (retry = 0; retry < 5; retry++) {
				printf("Bad hash, retrying read (%d/5) ... ",
				       retry + 1);
				if (info->read(info,
					       sector + get_aligned_image_offset(info, offset),
					       nr_sectors, (void *)load_ptr) != nr_sectors)

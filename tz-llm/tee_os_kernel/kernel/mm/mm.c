...
static void init_buddy_for_one_physmem_map(int physmem_map_idx)
{
    ...
    BUG_ON(KSTACK_BASE >= phys_to_virt(free_mem_start)
           && KSTACK_BASE < phys_to_virt(free_mem_end));
#endif
    npages =
    ...
    kdebug(
        "page_meta_start: 0x%lx, npages: 0x%lx, meta_size: 0x%lx, free_page_start: 0x%lx\n",
        page_meta_start,
        npages,
        sizeof(struct page),
        free_page_start);

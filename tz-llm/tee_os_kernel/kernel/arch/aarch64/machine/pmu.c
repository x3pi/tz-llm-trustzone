...
#define PMCR_EL0_X \
    (1 << 4) /* Enable export of events in an IMPLEMENTATION DEFINED event stream */
#define PMCR_EL0_DP \
...
#define PMCR_EL0_N_SHIFT \
    (11) /* An RO field that indicates the number of event counters implemented */
#define PMCR_EL0_N_MASK (0x1f)

/* PMUSERENR_EL0 Performance Monitors User Enable Register */
#define PMUSERENR_EL0_EN \
    (1                   \
     << 0) /* Traps EL0 accesses to the Performance Monitors registers to EL1 */
...
{
    asm volatile(
        "msr pmuserenr_el0, %0" ::"r"(PMUSERENR_EL0_EN | PMUSERENR_EL0_SW

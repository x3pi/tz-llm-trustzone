...
static void construct_init_env(char *env, vaddr_t top_vaddr,
                               ...
                               int argc, char **argv, int envp_argc,
                               char **envp_argv, int pid,
                               unsigned long load_offset)
{
    ...
    env_buf_append_int_auxv(&env_buf, AT_PHDR, meta->phdr_addr + load_offset);
...
int launch_process_with_pmos_caps(struct launch_process_args *lp_args)
{
    ...
    unsigned long stack_size = lp_args->stack_size;
    ...
    if (nr_caps > 0) {
        ...
        ret = usys_transfer_caps(new_process_cap, caps, nr_caps, transfer_caps);
        if (ret != 0) {
    ...
    /* Step-3: Create pmo of the stack, and a forbidden pmo to detect stack overflow. */
    ...
    seg_pmos_map_requests[0].addr = MAIN_THREAD_STACK_BASE;
    ...
    seg_pmos_map_requests[1].addr = MAIN_THREAD_STACK_BASE - PAGE_SIZE;
    ...
    for (i = 0; i < user_elf->segs_nr; ++i) {
        ...
        seg_pmos_map_requests[DEFAULT_PMO_CNT + i].addr = ROUND_DOWN(
            user_elf->user_elf_segs[i].p_vaddr + load_offset, PAGE_SIZE);
        seg_pmos_map_requests[DEFAULT_PMO_CNT + i].perm =
            user_elf->user_elf_segs[i].perm;
    }

    ret = map_pmos(
        new_process_cap, (void *)seg_pmos_map_requests, DEFAULT_PMO_CNT + i);
    if (ret != 0) {
    ...
    if (nr_pmo_map_reqs) {
        ret = map_pmos(new_process_cap, (void *)pmo_map_reqs, nr_pmo_map_reqs);
        if (ret != 0) {
            printf("%s: fail to map_pmos (ret: %d)\n", __func__, ret);
            goto fail;
        }

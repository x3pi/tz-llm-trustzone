    ...
    if (segs > 0) {
        for (int i = 0; i < segs; i++) {
            if (user_elf->user_elf_segs[i].elf_pmo > 0) {
                usys_revoke_cap(user_elf->user_elf_segs[i].elf_pmo, false);
            }
        }
                            ...
                            void *param, struct elf_file **elf_file)
{
    ...
    MALLOC_OR_FAIL(ph_buf, header->e_phnum * sizeof(struct elf_program_header));
    MALLOC_OR_FAIL(sh_buf, header->e_shnum * sizeof(struct elf_section_header));
    MALLOC_OR_FAIL(elf, sizeof(struct elf_file));
    MALLOC_OR_FAIL(ph_file_buf, header->e_phnum * (size_t)header->e_phentsize);
    MALLOC_OR_FAIL(sh_file_buf, header->e_shnum * (size_t)header->e_shentsize);
    ...
    for (int i = 0; i < header->e_phnum; i++) {
        ret = parse_elf_program_header(ph_file_buf
                                           + (ptrdiff_t)header->e_phentsize * i,
                                       header,
                                       &ph_buf[i]);
        if (ret < 0) {
            goto out_fail;
    ...
    for (int i = 0; i < header->e_shnum; i++) {
        ret = parse_elf_section_header(sh_file_buf
                                           + (ptrdiff_t)header->e_shentsize * i,
                                       header,
                                       &sh_buf[i]);
        if (ret < 0) {
            goto out_fail;
                                ...
                                struct user_elf **user_elf)
{
    ...
    memset(elf->user_elf_segs, 0, loadable_segs * sizeof(struct user_elf_seg));

    /**
     * Loop over all segments of elf_file(controlled by i), find and load
     * each loadable segment(controlled by j), and fill its user_elf_seg structure.
     */
    for (int i = 0, j = 0; i < elf_file->header.e_phnum; i++) {
        ...
        ret = usys_create_pmo(seg_map_sz, PMO_DATA);
        if (ret < 0) {
            goto out_fail;
        }
        ...
        seg_pmo = ret;
        seg_buf =
            chcore_auto_map_pmo(seg_pmo, seg_map_sz, VMR_READ | VMR_WRITE);
        if (!seg_buf) {
            ret = -errno;
        ...
        /**
         * With former chcore_auto_map_pmo, we directly load content of the
         * range into the pmo, without an intermediate buffer.
         */
        ret = loader(param, cur_ph->p_offset, seg_file_sz, seg_load_start);
        if (ret < 0) {
        ...
        if (cur_user_elf_seg->perm & VMR_EXEC) {
            usys_cache_flush(
                (unsigned long)seg_load_start, seg_file_sz, SYNC_IDCACHE);
        }
     ...
     * we have to use a separate pmo to store and map the program header table.
     * But actually, the PT_LOAD segments have already stored all data required
     * in runtime of the ELF file. Especially, the first PT_LOAD segment would
     * contains the ELF header and program headers of the ELF file, i.e., the
     * p_offset of this segment would be 0. So the virtual address of this
     * segment is the position of the start of the ELF file in the memory, and
     * the address of program header table can be calculated by adding this
     * virtual address with phoff from the ELF header.
     */
...
int load_elf_header_from_fs(const char *path, struct elf_header **elf_header)
{
    ...
    fd = ret;
    ret = __load_elf_header(file_range_loader, (void *)(long)fd, elf_header);

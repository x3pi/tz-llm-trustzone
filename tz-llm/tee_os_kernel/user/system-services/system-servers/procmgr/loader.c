...
constructor_t __find_loader_constructor(const char *path)
{
    for (int i = 0; i < CONSTRUCTOR_CNT; i++) {
        if (strncmp(path, constructor_table[i].path, LOADER_PATH_MAX_LENGTH)
            == 0) {
...
#define LIBC_LDSO_OFFSET (0x40000000UL)
#else
#define LIBC_LDSO_OFFSET (0x400000000000UL)
#endif
                                        ...
                                        struct launch_process_args *lp_args)
{
    ...
    printf(
        "CHCORE_LOADER warning: loader itself is loaded at a magic address\n");
     ...
     * process.
     */
    lp_args->load_offset = LIBC_LDSO_OFFSET;

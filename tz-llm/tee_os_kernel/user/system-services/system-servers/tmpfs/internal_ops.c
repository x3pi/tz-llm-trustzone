...
static int tmpfs_file_truncate(struct inode *reg, off_t len)
{
    ...
    if (len == 0) {
        ...
        if (page_off) {
            ...
            page = radix_get(&reg->data, page_no);
            if (page) {
                to_write = MIN(reg->size - len, PAGE_SIZE - page_off);
 ...
 */
static struct dentry *tmpfs_alloc_dent()
{
...
static bool tmpfs_dir_empty(struct inode *dir)
{
    ...
    for_each_in_htable (iter, i, node, &dir->dentries) {
        if ((ret = strcmp(iter->name.str, ".")) != 0
            && strcmp(iter->name.str, "..") != 0) {
            return false;
        }
                                      ...
                                      size_t len)
{
    ...
    for_each_in_hlist (iter, node, head) {
        if (iter->name.len == len && strncmp(iter->name.str, name, len) == 0) {
            return iter;
        }
    }
                          ...
                          void *end, int *read_bytes)
{
    ...
    for_each_in_htable (iter, b, node, &dir->dentries) {
        if (cnt >= start) {
            ...
            ret =
                tmpfs_dir_fill_dirent(&p, end, iter->name.str, cnt, type, ino);

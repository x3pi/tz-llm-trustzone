...
static const char *walk_component(struct nameidata *nd, bool trailing)
{
    ...
    dentry =
        i_parent->d_ops->dirlookup(i_parent, nd->last.str, (int)nd->last.len);
...
static struct dentry *lookup_create(struct nameidata *nd, unsigned open_flags)
{
    ...
    dentry = i_dir->d_ops->dirlookup(i_dir, nd->last.str, (int)nd->last.len);
    if (dentry) {
        return dentry;
    }
    ...
    if (open_flags & O_CREAT) {
        ...
        err =
            i_dir->d_ops->add_dentry(i_dir, dentry, nd->last.str, nd->last.len);
        if (err) {
        ...
        int err = i_dir->d_ops->mknod(i_dir, dentry, faked_mode, FS_REG);
        if (err) {
                  ...
                  struct dentry **dentry)
{
    ...
    while (!(err = walk_prefix(path, nd)) && (path = lookup_last(nd)) != NULL) {
        ;
    }

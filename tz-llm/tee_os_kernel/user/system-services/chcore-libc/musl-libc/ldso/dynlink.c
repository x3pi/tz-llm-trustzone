...
hidden void (*const __init_array_start)(void) = 0,
                    (*const __fini_array_start)(void) = 0;

extern hidden void (*const __init_array_end)(void),
...
static Sym *gnu_lookup(uint32_t h1, uint32_t *hashtab, struct dso *dso,
                       const char *s)
{
...
static Sym *gnu_lookup_filtered(uint32_t h1, uint32_t *hashtab, struct dso *dso,
...
static void do_relocs(struct dso *dso, size_t *rel, size_t rel_size,
                      size_t stride)
{
...
#ifdef TLS_ABOVE_TP
                reloc_addr[1] =
                    tls_val + def.dso->tls.offset + TPOFF_K + addend;
#else
...
static void *mmap_fixed(void *p, size_t n, int prot, int flags, int fd,
                        off_t off)
{
    flags &= (~MAP_FIXED);
...
static void *map_library(int fd, struct dso *dso)
{
    ...
    for (i = eh->e_phnum; i; i--, ph = (void *)((char *)ph + eh->e_phentsize)) {
        if (ph->p_type == PT_DYNAMIC) {
            ...
            if (!runtime && ph->p_memsz > __default_stacksize) {
                __default_stacksize = ph->p_memsz < DEFAULT_STACK_MAX ?
                                          ph->p_memsz :
                                          DEFAULT_STACK_MAX;
            }
        }
    ...
    if (DL_FDPIC && !(eh->e_flags & FDPIC_CONSTDISP_FLAG)) {
        dso->loadmap = calloc(
            1, sizeof *dso->loadmap + nsegs * sizeof *dso->loadmap->segs);
        if (!dso->loadmap)
            goto error;
        ...
        for (ph = ph0, i = 0; i < nsegs;
            ...
            prot = (((ph->p_flags & PF_R) ? PROT_READ : 0)
                    ...
                    | ((ph->p_flags & PF_X) ? PROT_EXEC : 0));
            map = mmap(0,
                       ph->p_memsz + (ph->p_vaddr & PAGE_SIZE - 1),
                       prot,
                       MAP_PRIVATE,
                       fd,
            ...
            dso->loadmap->segs[i].addr =
                (size_t)map + (ph->p_vaddr & PAGE_SIZE - 1);
            ...
            if (prot & PROT_WRITE) {
                ...
                size_t pgend = brk + ph->p_memsz - ph->p_filesz + PAGE_SIZE - 1
                               & -PAGE_SIZE;
                if (pgend > pgbrk
                    && mmap_fixed(map + pgbrk,
                                  pgend - pgbrk,
                                  prot,
...
#if CHCORE_MMAP == 1
    map = DL_NOMMU_SUPPORT ? mmap((void *)addr_min,
                                  map_len,
...
static int path_open(const char *name, const char *s, char *buf,
                     size_t buf_size)
{
    size_t l;
    int fd;
...
static struct dso *load_library(const char *name, struct dso *needed_by)
{
    ...
    if (p->tls.image) {
        ...
        p->tls.offset =
            tls_offset
            + ((p->tls.align - 1) & (-tls_offset + (uintptr_t)p->tls.image));
 ...
 */
#if CHCORE_GDB_AUTO_LOAD_LIB
__attribute__((noinline)) void gdb_loadefl_break()
{
...
static void reloc_all(struct dso *p)
{
    ...
    for (; p; p = p->next) {
        ...
        if (head != &ldso && p->relro_start != p->relro_end
            && mprotect(laddr(p, p->relro_start),
...
void __dls3(size_t *sp, size_t *auxv)
{
    ...
    if (aux[AT_PHDR] != (size_t)ldso.phdr) {
        ...
        if (!ehdr) {
            dprintf(
                2, "%s: %s: Not a valid dynamic program\n", ldname, argv[0]);
            _exit(1);
        }
        close(fd);
    ...
    if (search_vec(auxv, &vdso_base, AT_SYSINFO_EHDR) && vdso_base) {
        ...
        for (i = ehdr->e_phnum; i;
            ...
            if (phdr->p_type == PT_LOAD)
                vdso.base =
                    (void *)(vdso_base - phdr->p_vaddr + phdr->p_offset);
        }
        vdso.name = "";
    ...
    printf("%s %d argv-1 %p *(argv-1) %ld *argv %s\n",
           __func__,
           __LINE__,
           argv - 1,
           *(long *)(argv - 1),
    ...
    if (!strcmp("/gtask.elf", *argv) || !strcmp("/teesmcmgr.elf", *argv)
...
static void prepare_lazy(struct dso *p)
{
    ...
    if (dyn[DT_BIND_NOW] || (dyn[DT_FLAGS] & DF_BIND_NOW)
        || (flags1 & DF_1_NOW))
        return;
...
static void *do_dlsym(struct dso *p, const char *s, void *ra)
{
    ...
    if ((def.sym->st_info & 0xf) == STT_TLS)
        return __tls_get_addr(
            (tls_mod_off_t[]){def.dso->tls_id, def.sym->st_value - DTP_OFFSET});
...
hidden void *__dlsym(void *restrict p, const char *restrict s,
                     void *restrict ra)
{
    void *res;
    pthread_rwlock_rdlock(&lock);
...
hidden void *__dlsym_redir_time64(void *restrict p, const char *restrict s,
                                  void *restrict ra)
{
#if _REDIR_TIME64

...
#define TZASC_NR               (4)
/* Must match arch/arm64/mm/init.c's TZASC_TOTAL_MEM_SIZE (see the
 * detailed comment there -- attempts to grow this past 3GB to fit a
 * full model in one pass, per the TZ-LLM paper's "extend and shrink"
 * design, hit a physical-memory-fragmentation ceiling on this board).
 * This constant only feeds g_tzasc_cma_meta->size, which userspace
 * reads to know how much of each TZASC region is actually usable; it
 * must track the real reservation. */
...
int tzasc_cma_push_pages_with_index(unsigned long size, int cma_index) {
	...
	/*
	 * BUG FIX: `flags` was declared but tzasc_cma_lock was never actually
	 * acquired here, despite being declared/initialized elsewhere
	 * specifically for this structure. g_tzasc_cma_meta->count++ is a
	 * non-atomic read-modify-write; this function can be entered
	 * concurrently from multiple CPU cores both via the CA's direct
	 * ioctl(PUSH_PAGES) path and via the SMC_EXIT_SHADOW relay in
	 * smc_call_cpu_resume() serving ChCore's own compute threads (up to
	 * 4 of them, each independently issuing push_pages for a different
	 * tensor while restoring parameters in parallel). Without a lock,
	 * two threads allocating in the same cma_index concurrently can both
	 * read the same `count`, both write into the same entry[count] slot
	 * (one clobbering the other's cma_pages/paddr/size), and both then
	 * believe they own that entry_index - silently cross-linking one
	 * tensor's TA-side mapping to a different tensor's physical pages
	 * (or to pages the CA never actually wrote real data into). This was
	 * the root cause of most weight tensors reading back stale/wrong
	 * bytes while a few "lucky" (non-colliding) ones came through intact.
	 * cma_alloc() itself is already safe to call without holding this
	 * lock (it has its own internal locking), so only the entry
	 * bookkeeping needs to be serialized here.
	 */
	new_pages = tzasc_cma_alloc_with_index(cma_index, size);
	if (new_pages == NULL)
		return -ENOMEM;

	spin_lock_irqsave(&tzasc_cma_lock, flags);
	entry = &g_tzasc_cma_meta->entry[g_tzasc_cma_meta->count];
	entry->cma_pages = new_pages;
	...
	spin_unlock_irqrestore(&tzasc_cma_lock, flags);
...
int tzasc_cma_pop_pages_with_index(int cma_index) {
	...
	unsigned long flags;
	struct tzasc_cma_meta *g_tzasc_cma_meta = g_tzasc_cma_meta_arr + cma_index;
	struct tzasc_cma_entry entry_copy;

	if (cma_index < 0 || cma_index >= TZASC_NR)
		return -EINVAL;

	/*
	 * Same unprotected-count bug as tzasc_cma_push_pages_with_index():
	 * count-- and the entry read must be serialized against concurrent
	 * push/pop on the same cma_index, or two callers can decrement past
	 * each other and free/return the wrong entry.
	 */
	spin_lock_irqsave(&tzasc_cma_lock, flags);
	...
	entry_copy = g_tzasc_cma_meta->entry[g_tzasc_cma_meta->count];
	spin_unlock_irqrestore(&tzasc_cma_lock, flags);

	// pr_info("%s %d: size %lx count %d\n", __func__, __LINE__, entry_copy.size, ret);
	ret = tzasc_cma_free_with_index(cma_index, entry_copy.cma_pages, entry_copy.size);
...
int tzasc_cma_init(void) {
	...
	for (cma_index = 0; cma_index < TZASC_NR; cma_index++) {
		...
		/* cma_declare_contiguous() in arch/arm64/mm/init.c doesn't
		 * check its own return value, so tzasc_cma[cma_index] can be
		 * NULL here if the kernel couldn't find enough contiguous
		 * memory for this region. Skip it instead of crashing. */
		if (tzasc_cma[cma_index] == NULL) {
			tloge("%s: tzasc_cma[%d] is NULL, skipping\n", __func__, cma_index);
			g_tzasc_cma_meta->base = 0;
			g_tzasc_cma_meta->size = 0;
...
/*
 * BUG FIX: this was a plain global `unsigned long on_fly_io_thread;` with no
 * locking, used to hand a ChCore thread pointer from one
 * smc_call_cpu_resume() invocation to the NEXT invocation on the exit_reason
 * == 4 (IO step) path. Up to 4 ca_thread pthreads (one per pinned CPU core,
 * see "bind to core N" in main.cpp) each independently call this via the
 * llm_run ioctl in their own busy-poll loop - with a bare global, thread A's
 * pending value could be read-and-cleared by thread B's concurrent call
 * before A ever got to consume it, silently losing which ChCore thread was
 * waiting to be resumed. That thread then never receives its resume SMC and
 * stays parked forever - observed on hardware as: all userspace worker
 * threads spinning at ~400% CPU (busy-polling their own is_finished() loop),
 * zero forward progress, no kernel panic/error, no dmesg output, exactly
 * matching "some ChCore thread got silently dropped" rather than a crash.
 * Fixed by making it per-CPU, matching the pinned-core design the userspace
 * side already assumes.
 */
static DEFINE_PER_CPU(unsigned long, on_fly_io_thread);
...
unsigned long smc_call_cpu_resume(struct out_result *result) {
	 ...
	 * returning to the ioctl() caller. cond_resched() alone only actually
	 * switches away - and only then reports an RCU quiescent state - if
	 * something else is already runnable on this core; under NPU+TrustZone
	 * combined load with nothing else scheduled on these pinned cores it
	 * silently no-ops every time, so this loop can run for tens of seconds
	 * without RCU ever seeing a quiescent state on this CPU. Observed on
	 * hardware as `rcu_preempt detected stalls` -> cascading soft-lockup

...
next:
	...
	/*
	 * set_soc_info() is what actually calls dev_pm_opp_set_supported_hw()
	 * so the OPP core can match each opp-supported-hw entry against this
	 * chip's bin/volt_sel. Without this call, opp_table->supported_hw is
	 * never set, and _opp_is_supported() unconditionally rejects every
	 * OPP node that carries an opp-supported-hw property (see
	 * drivers/opp/of.c), so dev_pm_opp_of_add_table() below fails with
	 * "no supported OPPs" even though the DT data is fine.
	 */

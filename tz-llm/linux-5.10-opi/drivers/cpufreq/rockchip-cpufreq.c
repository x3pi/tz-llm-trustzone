...
{
	/*
	 * rockchip_opp_dump_cur_state() -> dev_pm_opp_get_opp_table() and
	 * regulator_get_voltage() both take sleeping mutexes
	 * (opp_table_lock / regulator_lock_dependent()). panic_notifier_list
	 * callbacks run with IRQs disabled and other CPUs stopped, so any
	 * mutex_lock() here either trips "BUG: scheduling while atomic" or
	 * deadlocks outright if the lock was held by a CPU that panic()
	 * already stopped mid-critical-section - turning a recoverable
	 * watchdog/panic event into an unrecoverable hang. The cur-state
	 * dump is diagnostic only, so skip it entirely rather than risk
	 * crashing the panic path itself.
	 */
	pr_emerg("rockchip-cpufreq: skipping cur-state dump in panic context (would sleep)\n");

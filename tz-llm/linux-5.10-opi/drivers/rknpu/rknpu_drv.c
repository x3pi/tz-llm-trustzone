...
extern void tzdriver_register_rknpu_dev(void *dev);
extern void rknpu_job_next_work_s(struct work_struct *work);
...
static int rknpu_probe(struct platform_device *pdev)
{
	...
	tzdriver_register_rknpu_dev(rknpu_dev);
	if (!rknpu_dev) {
	...
	if (rknpu_dev->iommu_en) {
		...
		/*
		 * Even in IOMMU mode, RKNPU_MEM_CREATE still tries a
		 * DMA_ATTR_FORCE_CONTIGUOUS allocation first (only falling
		 * back to non-contiguous IOMMU-backed pages on failure, a
		 * fallback path with its own "failed to map pages into
		 * vma: -6" bug on this kernel). Bind a dedicated
		 * memory-region if the board DTS provides one so that first
		 * contiguous attempt doesn't have to compete with other
		 * CMA users for the tiny default pool.
		 */
		if (of_find_property(dev->of_node, "memory-region", NULL)) {
			...
			if (!ret) {
				LOG_DEV_INFO(
					dev,
					"initialize reserved memory for rknpu device (iommu mode)!\n");
			}
		}
	} else {
	...
	for (i = 0; i < config->num_irqs; i++) {
		...
		rknpu_dev->subcore_datas[i].rknpu_dev = rknpu_dev;
		rknpu_dev->subcore_datas[i].core_index = i;
		INIT_WORK(&rknpu_dev->subcore_datas[i].resume_work,
			  rknpu_job_next_work_s);
...
int rknpu_init_test(void)
{
	return platform_driver_register(&rknpu_driver);
}

void rknpu_exit_test(void)
{
	platform_driver_unregister(&rknpu_driver);
}

EXPORT_SYMBOL(rknpu_init_test);
EXPORT_SYMBOL(rknpu_exit_test);

...
int main() {
    std::cout << "[Xapian-TA] Starting inside TEE!" << std::endl;

    // 1. Initialize Xapian Database (In-Memory to avoid file I/O & socketpair syscalls in TEE)
    ...
    if (vaddr) {
        ...
        // Yield to Linux and wait for the first actual query
        std::cout << "[Xapian-TA] Initialization complete. Yielding and waiting for query..." << std::endl;

...
if [ "$setup" = "base" ]; then
    ...
    echo "111: $model#$len"
    LD_LIBRARY_PATH=/data/local/tmp/rknpu/ /data/local/tmp/rknpu/ld-linux-aarch64.so.1 /data/local/tmp/rknpu/llama-cli --no-warmup -m $model_path -p "$model#$len" -n $n --cache $cache -s 1 -ngl 100 -t 4 --no-mmap -c 1124 &> /dev/null
elif [ "$setup" = "tz" ]; then
    echo "Running TZ-LLM..."

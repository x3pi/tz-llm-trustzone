...
echo "Fast build uboot - avoid removing and re-copying out/uboot/src_tmp."
if [ ! -d ${UBOOT_SRC_TMP_PATH} ];then
	echo "ERROR: dir ${UBOOT_SRC_TMP_PATH} not found. Maybe you need to run normal uboot build first"
       	popd
	exit
fi

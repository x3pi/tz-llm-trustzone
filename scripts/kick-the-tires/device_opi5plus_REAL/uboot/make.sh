...
CROSS_COMPILE_ARM64=../../../prebuilts/gcc/linux-x86/aarch64/gcc-linaro-7.5.0-2019.12-x86_64_aarch64-linux-gnu/bin/aarch64-linux-gnu-
########################################### User not touch #############################################
...
function pack_uboot_itb_image()
{
	...
	if [ "${ARM64_TRUSTZONE}" == "y" ]; then
		...
		if grep BL32_OPTION -A 1 ${INI} | grep SEC=1 ; then
			cp ${RKBIN}/../../../../base/tee/tee_os_kernel/kernel/bl32.bin tee.bin
